 ...
 * @access  Private
 */
router.get("/get-keys", verificationMiddleware, async (req, res, next) => {
  try {
    ...
    const apiKeys = await ApiKey.find({
      $or: [{ userId }, ...(tenantIds.length ? [{ tenantId: { $in: tenantIds } }] : [])],
    }).lean();
    ...
    const apiKeyTenantIds = [...new Set(apiKeys.map((k) => String(k.tenantId)).filter(Boolean))];
