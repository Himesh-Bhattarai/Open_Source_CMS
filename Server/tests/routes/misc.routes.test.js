...
describe("Misc mounted routes", () => {
  test("GET integrationsApi valid invalid edge", async () => {
    const router = await loadMiscRouter("../../Routes/integrationsApi/integrationsApi.js", [
      [
        "../../CheckPoint/Integrations/integrationsApi.js",
        () => ({ integrationsApi: (req, res) => res.status(200).json({ ok: true }) }),
      ],
    ]);
    const app = createRouteTestApp("/integrations", router);

    expect(
      (await request(app).get("/integrations/get-api").set("Cookie", makeAuthCookie())).status,
    ).toBe(200);
    ...
    const edgeRouter = await loadMiscRouter("../../Routes/integrationsApi/integrationsApi.js", [
      [
        "../../CheckPoint/Integrations/integrationsApi.js",
        () => ({ integrationsApi: (req, res, next) => next(new Error("integration fail")) }),
      ],
    ]);
    const edgeApp = createRouteTestApp("/integrations", edgeRouter);
    expect(
      (await request(edgeApp).get("/integrations/get-api").set("Cookie", makeAuthCookie())).status,
    ).toBe(500);
  });
  ...
  test("GET api keys valid invalid edge", async () => {
    ...
    expect((await request(app).get("/keys/get-keys").set("Cookie", makeAuthCookie())).status).toBe(
      200,
    );
    ...
    expect(
      (await request(edgeApp).get("/keys/get-keys").set("Cookie", makeAuthCookie())).status,
    ).toBe(500);
  });
  ...
  test("GET stats valid invalid edge", async () => {
    const router = await loadMiscRouter("../../Routes/Stats/Stats.js", [
      [
        "../../Models/Page/Page.js",
        () => ({ Page: { countDocuments: jest.fn().mockResolvedValue(1) } }),
      ],
      [
        "../../Models/Blog/Blogpost.js",
    ...
    expect(
      (await request(app).get("/stats/stats/For-Dashboard").set("Cookie", makeAuthCookie())).status,
    ).toBe(200);
    ...
    const edgeRouter = await loadMiscRouter("../../Routes/Stats/Stats.js", [
      [
        "../../Models/Page/Page.js",
        () => ({ Page: { countDocuments: jest.fn().mockRejectedValue(new Error("db fail")) } }),
      ],
      [
        "../../Models/Blog/Blogpost.js",
        () => ({ BlogPost: { countDocuments: jest.fn().mockResolvedValue(2) } }),
  ...
  test("Notifications valid invalid edge", async () => {
    ...
    expect(
      (await request(app).get("/noti/get-notification").set("Cookie", makeAuthCookie())).status,
    ).toBe(200);
  ...
  test("Admin load current behavior typo path", async () => {
    const router = await loadMiscRouter("../../Routes/Load/adminLoad.js", [
      [
        ...
        () => ({ User: { findOne: jest.fn().mockResolvedValue({ _id: "u1", role: "admin" }) } }),
      ],
    ]);
    const app = createRouteTestApp("/admin", router);

    const validCurrent = await request(app)
    ...
    const edge = await request(app)
      .get("/admin/get-all-users")
      .set("Cookie", makeAuthCookie({ useId: "u1" }));
  ...
  test("Feedback route valid invalid edge", async () => {
    const router = await loadMiscRouter("../../Services/feedBack.js", [
      [
        ...
        () => ({ Feedback: { create: jest.fn().mockResolvedValue({ _id: "f1" }) } }),
      ],
      [
        "../../Models/Client/User.js",
        () => ({
          User: { findOne: jest.fn().mockResolvedValue({ name: "U", email: "u@example.com" }) },
        }),
      ],
    ]);
    ...
    expect(
      (
        await request(app)
          .post("/feedback/user/collect")
          .set("Cookie", makeAuthCookie())
          .send({ msg: "hello" })
      ).status,
    ).toBe(200);
    expect((await request(app).post("/feedback/user/collect").send({ msg: "hello" })).status).toBe(
      401,
    );
    expect(
      (await request(app).post("/feedback/user/collect").set("Cookie", makeAuthCookie()).send({}))
        .status,
    ).toBe(500);
  });
        ...
        () => ({
          User: {
            findOne: jest.fn().mockResolvedValue({
              password: "$2b$10$5fd2wQ4vUpStxK4NdVSzUuXQXoYzKxuxMxDPZWS9Vyuk3F7S3w7Dq",
              save: jest.fn(),
            }),
          },
        }),
    ...
    expect(
      (
        await request(app)
          .post("/pwd/change-password")
          .set("Cookie", makeAuthCookie())
          .send({ oldPassword: "123", newPassword: "12" })
      ).status,
    ).toBe(400);
  });
  ...
  test("Validate user payload valid invalid edge", async () => {
    const router = await loadMiscRouter("../../Services/validateUser.js", [
      [
        ...
        () => ({
          User: {
            findOne: jest.fn().mockResolvedValue({ email: "u@example.com", password: "hashed" }),
          },
        }),
      ],
    ...
    expect(
      (
        await request(app)
          .post("/validate/user-payload")
          .set("Cookie", makeAuthCookie())
          .send({ par1: "u@example.com", par2: "12345678" })
      ).status,
    ).toBe(200);
    expect(
      (
