...
const loadOneRoutes = async () => {
  jest.resetModules();
  const route = (name) => express.Router().get("/", (req, res) => res.status(200).json({ route: name }));
...
describe("External API routes", () => {
  ...
  test("External media valid invalid edge (/: quirk)", async () => {
    const pass = (req, res, next) => {
      req.tenant = { _id: "t1" };
      next();
    };
  ...
  test("oneRoutes wiring valid invalid edge", async () => {
    ...
    jest.resetModules();
    jest.unstable_mockModule("../../Routes/Api/getBlog.js", () => ({ default: express.Router().get("/", (req, res, next) => next(new Error("blog route fail"))) }));
    jest.unstable_mockModule("../../Routes/Api/getPages.js", () => ({ default: express.Router().get("/", (req, res) => res.status(200).json({ ok: true })) }));
    jest.unstable_mockModule("../../Routes/Api/getMenu.js", () => ({ default: express.Router().get("/", (req, res) => res.status(200).json({ ok: true })) }));
    jest.unstable_mockModule("../../Routes/Api/getFooter.js", () => ({ default: express.Router().get("/", (req, res) => res.status(200).json({ ok: true })) }));
    jest.unstable_mockModule("../../Routes/Api/getSeo.js", () => ({ default: express.Router().get("/", (req, res) => res.status(200).json({ ok: true })) }));
    jest.unstable_mockModule("../../Routes/Api/getTheme.js", () => ({ default: express.Router().get("/", (req, res) => res.status(200).json({ ok: true })) }));
    jest.unstable_mockModule("../../Routes/Api/getMedia.js", () => ({ default: express.Router().get("/", (req, res) => res.status(200).json({ ok: true })) }));
    jest.unstable_mockModule("../../Routes/Api/getForm.js", () => ({ default: express.Router().get("/", (req, res) => res.status(200).json({ ok: true })) }));
